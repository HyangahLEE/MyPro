...
public class Ask2DAO {

private static Ask2DAO dao = null;
	
	private Ask2DAO() {}
	
	public static Ask2DAO getInstance() {
		if( dao == null ) {
			dao = new Ask2DAO();
		}
		return dao;
	...
	public int insert(Connection conn, AskDTO dto) {
		...
		public List<AskDTO> selectList(
			...
			sql+=" from ( ";
			sql+="          select * from ask ";
			sql +=" where " + searchCondition +" like '%" + searchWord+"%' and adid like 'adId0002' ";
	...
	public List<AskDTO> selectList(
		...
		sql+= ", case       when ( sysdate - awritedt ) < 0.041667  then  'true'      else 'false'      end  new ";
		sql+=" from ( ";
		sql+="          select * from ask where adid like 'adId0002' ";
	    sql+=" order by ref desc, step asc  ";
		sql+="         ) ";
		sql+=")";
		sql+=" where  rnum between ? and ? ";		
	...
	public int getTotalRecords(Connection conn) {
		int cnt = 0;
		String sql = "select count(*) cnt from ask where adid like 'adId0002' ";
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='ask2_list.do?page=%d&searchCondition=%s&searchWord=%s'>%d</a>", i,searchCondition,searchWord,i);
		}
		paging +="</div>";
		return paging; 
	}
