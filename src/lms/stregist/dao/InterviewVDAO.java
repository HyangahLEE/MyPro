...
public class InterviewVDAO {

	private static InterviewVDAO dao = null;
	
	private InterviewVDAO() {}
	
	public static InterviewVDAO getInstance() {
		if( dao == null ) {
			dao = new InterviewVDAO();
		}
		return dao;
	}		
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='regist_view.do?tpid=%s&inpage=%d'>%d</a>",tpid, i,i);
		}
		paging +="</div>";
		return paging; 
	}
