...
public class InterviewL2DAO {

	private static InterviewL2DAO dao = null;
	
	private InterviewL2DAO() {}
	
	public static InterviewL2DAO getInstance() {
		if( dao == null ) {
			dao = new InterviewL2DAO();
		}
		return dao;
	}		
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='regist2_list.do?page=%d'>%d</a>", i,i);
		}
		paging +="</div>";
		return paging; 
	}
