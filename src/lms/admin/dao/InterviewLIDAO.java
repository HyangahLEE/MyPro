...
public class InterviewLIDAO {

	private static InterviewLIDAO dao = null;
	
	private InterviewLIDAO() {}
	
	public static InterviewLIDAO getInstance() {
		if( dao == null ) {
			dao = new InterviewLIDAO();
		}
		return dao;
	}		
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='interview_list.do?page=%d'>%d</a>", i,i);
		}
		paging +="</div>";
		return paging; 
	}
