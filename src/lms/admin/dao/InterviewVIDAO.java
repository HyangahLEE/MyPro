...
public class InterviewVIDAO {

	private static InterviewVIDAO dao = null;
	
	private InterviewVIDAO() {}
	
	public static InterviewVIDAO getInstance() {
		if( dao == null ) {
			dao = new InterviewVIDAO();
		}
		return dao;
	}		
	...
	public List<InterviewVDTO> selectList(
		...
		sql+=" from ( ";
		sql+="          select  a.memberid memberid , mname, mbirth, iresult  from iapplication a inner join member b on a.memberid = b.memberid ";
	    sql+=" where tpid like ? ";
		sql+=" order by iacid desc ";
		sql+="         ) ";
		sql+=")";
	...
	public int getTotalRecords(Connection conn, String tpid) {
		int cnt = 0;
		String sql = "select count(*) cnt from iapplication ";
		       sql +=" where tpid = ? " ;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='interview_view.do?tpid=%s&inpage=%d'>%d</a>",tpid, i,i);
		}
		paging +="</div>";
		return paging; 
	}
	...
	// 업데이트. 결과 정보 저장
		...
		public int update(Connection conn, InterviewVDTO dto) {
			...
			String sql = "update iapplication "
					+" set iresult = ? "
					+" where memberid = ?";
