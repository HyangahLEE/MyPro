...
private static ReviewDAO dao = null;
	...
	public int insert(Connection conn, ReviewDTO dto) {
		...
		public List<ReviewDTO> selectList(
			...
			sql +=" where " + searchCondition +" like '%" + searchWord+"%' and rdid like 'rdId0001' ";
	...
	public List<ReviewDTO> selectList(
		...
		sql+= ", case       when ( sysdate - rwritedt ) < 0.041667  then  'true'      else 'false'      end  new ";
		sql+=" from ( ";
		sql+="          select * from review where rdid like 'rdId0001' ";
	 /*   sql+=" order by ref desc, step asc  ";*/
		sql+=" order by rid desc ";
		sql+="         ) ";
		sql+=")";
	...
	public int getTotalRecords(Connection conn) {
		int cnt = 0;
		String sql = "select count(*) cnt from review where rdid like 'rdId0001' ";
	...
	public String pagingProc(int currentPage, int pageSize, int pageBlock	// 1 2 3... 10
		...
		for( int i= blockStart; i<=blockEnd; i++) {
			...
			else paging +=String.format("<a href='review_list.do?page=%d&searchCondition=%s&searchWord=%s'>%d</a>", i,searchCondition,searchWord,i);
		}
		paging +="</div>";
		return paging; 
	}
